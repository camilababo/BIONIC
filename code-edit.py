...
def module_detection_eval():
    ...
    evaluations = pd.DataFrame(
        evaluations, columns=["Standard", "Dataset", "Module Match Score (ARI)"]
    )
    ...
    for method in config["methods"]:
        for metric in config["metrics"]:
            ...
            for t in np.linspace(0, np.max(maxdists(link)), num=config["thresholds"]):
                ...
                if score > best_score:
                    best_score = score
                    best_module_labels = cluster_labels

    # Create a dictionary to map each gene to its cluster label
    gene_cluster_map = dict(zip(features.index, best_module_labels))

    # Save the gene-cluster associations to a text file
    with open('feature-cluster-labels.txt', "w") as f:
        for gene, cluster_label in gene_cluster_map.items():
            f.write(f"{gene}\t{cluster_label}\n")
    ...
    for method in config["methods"]:
        for metric in config["metrics"]:
            ...
            for t in np.linspace(0, np.max(maxdists(link)), num=config["thresholds"]):
                ...
                if score > best_score:
                    best_score = score
                    est_cluster_labels = dict(zip(adjacency.index, cluster_labels))

        # Save the gene-cluster associations to a text file
        with open('network-cluster-labels.txt', "w") as f:
            for gene, cluster_label in best_cluster_labels.items():
                f.write(f"{gene}\t{cluster_label}\n")
...
def _evaluate(args, _):
    ...
    for name, feat in features_.items():
        ari_score = evaluate_features(feat, labels, config_standard)
        evaluations.append([standard_name, name, ari_score])

    for name, network in networks_.items():
        ari_score = evaluate_network(network, labels, config_standard)
        evaluations.append([standard_name, name, ari_score])
